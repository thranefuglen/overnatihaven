...
function readSchema(): string {
  // Try to read from filesystem first (local dev with tsx)
  const candidates = [
  ...
  for (const candidate of candidates) {
    try {
      ...
      // ignore filesystem errors (e.g. Vercel serverless)
    }
  }

  // Fallback: embedded schema for Vercel serverless where fs access is limited
